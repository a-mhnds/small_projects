...
company_name_len = len(df['Company'].unique())


#How many reviews are made in 2013 in the dataset?
rev_num_2013 = len(df[df['Review Date']==2013])


#In the BeanType Column, how many missing values are there?
Missed_BinType = df_temp['Bean Type'].isna().sum()


#Visualize the rating column with a histogram
rating_hist = sns.histplot(df['Rating'], stat='percent', label='Data Histogram', common_norm=True, kde=True)
...
scat_plt = plt.scatter(x=df['Cocoa Percent'], y=df['Rating'], alpha=0.1)
...
df['Rating'] = normalizer.fit_transform(df[['Rating']])
...
mean_lst = []
for i in names:
    mean_lst.append(float(df_temp.iloc[df[df['Company']==i].index,6].mean()))
...
company_by_rev = company_by_rev.sort_values(by='Avg_Rev_Score',ascending=False)
...
df_encoded = encoder.fit_transform(df[['Company', 'Company Location']])